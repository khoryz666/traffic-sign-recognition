...
# # 02 — HOG Feature Extraction — Chinese Traffic Signs
...
# ```
# 
# The loader below searches the dataset folder recursively, so it still works if
# KaggleHub creates an extra subfolder.
...
# The Kaggle annotation uses file_name, so this lets the
# notebook work even if images are inside a nested folder.
image_lookup = {
    p.name: p
...
#    Keeps the same interface used by the old GTSRB code:
...
# ### Proposal-Based Color & Shape Segmentation
...
# ### HOG using the proposal segmentation mask
...
from PIL import Image
from skimage.feature import hog
def extract_masked_hog_features(image, mask):
    # We extract the ROI first, like in extract_roi_from_mask
    ...
    # 1. Use LANCZOS for high-quality resizing to avoid aliasing artifacts
    roi_resized = roi_pil.resize((32, 32), Image.Resampling.LANCZOS)

    # Keep the image in RGB instead of converting to Grayscale
    roi_np = np.array(roi_resized)

    # 2. Extract HOG using all 3 color channels
    features, hog_image = hog(
        roi_np,
        orientations=9,
        pixels_per_cell=(4, 4),
        cells_per_block=(2, 2),
        block_norm='L2-Hys',       # Explicitly use L2-Hysteresis normalization
        transform_sqrt=True,       # Power law compression (helps with shadows)
        visualize=True,
        channel_axis=-1            # Instructs HOG to use all 3 RGB channels
    )
...
plt.title('HOG Image from Segmented Traffic Sign')
plt.axis('off')
plt.show()
...
# Select 10 evenly distributed training images.
# This is dynamic, so the indices stay valid even when
# the dataset size changes.
sample_indices = np.linspace(
    0,
    len(train_dataset) - 1,
    num=min(10, len(train_dataset)),
    dtype=int
).tolist()
...
# Keep axes consistently 2-dimensional when only one sample exists
...
# ## Full Dataset HOG Feature Extraction

# In[25]:


def extract_hog_features_from_image(image):
    # Convert PIL image to BGR for OpenCV functions
    image_cv = np.array(image.convert("RGB"))
    img_bgr = cv2.cvtColor(image_cv, cv2.COLOR_RGB2BGR)

    # Segment traffic sign
    mask_red = segment_red(img_bgr)
    mask_blue = segment_blue(img_bgr)
    mask_yellow = segment_yellow(img_bgr)
...
test_features = extract_hog_features_from_image(
    train_dataset[0][0]
)
...
for index in sample_indices:
    ...
    features = extract_hog_features_from_image(
        image
    )
...
# ## Extract HOG Features from the Full Training Dataset
...
for index in range(total_train):
    ...
    filename = train_dataset.annotations.iloc[index]["file_name"]
...
print("train_filenames:",train_filenames.shape)
...
print(
    ...
    TRAIN_FEATURE_FILE.resolve()
)


# ## Extract HOG Features from the Full Test Dataset
...
for index in range(total_test):
    ...
    filename = test_dataset.annotations.iloc[index]["file_name"]
...
print("test_filenames:",test_filenames.shape)
...
# 
# The Assignment 1 input images are used as the required external testing set if
# the `../data/Inputs` folder is available.
# 
# The same HOG feature extraction pipeline used for the **Chinese Traffic Signs**
# dataset is applied to these images.
# 
# The 20% Chinese Traffic Signs split is retained as the labelled dataset test set.
# 
...
# ### HOG Feature Extraction on Assignment 1 Images

# In[36]:


# Extract the same 1764-dimensional HOG features
...
assignment1_output = (
    FEATURE_DIR / "assignment1_test_hog_features.npz"
)
...
# 
# The final HOG feature extraction produces **1764 features per image** using
# 9 orientations, 4×4 pixels/cell and 2×2 cells/block.
# 
# The exact training/test counts and extraction success rates are printed by the
# cells above because the Chinese Traffic Signs dataset is split dynamically using
# a reproducible **80% training / 20% testing stratified split**.
# 
# The Chinese Traffic Signs dataset contains **58 categories**. The Assignment 1
# input images can still be processed as the required external test images when
# they are placed under `../data/Inputs`.
# 
# All successfully extracted labelled images are represented with the same
# 1764-dimensional HOG feature vector and saved as compressed `.npz` files for
# the later classifier stage.
# 
