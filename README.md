...
The pipeline runs through four numbered stages (`00-deps-and-dataset/` → `01-roi-segmentation/` → `02-feature-extraction/` → `03-classifier/`), sharing code through the [`pipeline/`](pipeline/) package rather than each notebook redefining its own copy. See the diagram above for the full data/code flow.
...
| Classes | 58 |
| Duplicate annotation rows removed | 166 |
| Orphan annotation rows removed (no matching image) | 0 |
| Held-out assignment-test images (never used for training) | 56 |
...
| Metric | Value |
| :--- | :--- |
| Traffic-sign contour detected | 56 / 56 |
| Shape breakdown | Circle 22 · Triangle 17 · Octagon 5 · Rectangle 4 · Square 2 · Unknown 6 |
...
| :--- | :-: | :-: | :-: | :-: |
| HOG | 1,764 | 4,749 / 4,753 | 1,189 / 1,189 | 56 / 56 |
| HSV histogram | 960 | 4,749 / 4,753 | 1,189 / 1,189 | 56 / 56 |

### 03 — Classifiers

Each classifier is tuned on the training split, evaluated on a stratified 20% test split (1,189 images), and finally evaluated on the fully blind 56-image held-out set.

| Classifier | Best parameters | Test accuracy | Test macro F1 | Held-out accuracy | Held-out macro F1 |
| :--- | :--- | :-: | :-: | :-: | :-: |
| KNN | k=1, manhattan, uniform | 0.9790 | 0.9771 | 0.9821 | 0.9753 |
| Logistic Regression | C=1.0 | 0.9790 | 0.9830 | 0.9821 | 0.9753 |
| SVM | rbf, C=10, gamma=scale, class_weight=balanced | 0.9807 | 0.9821 | 0.9821 | 0.9753 |
